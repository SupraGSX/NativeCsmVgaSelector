#!/usr/bin/env python3
"""Test real legacy-disk selection/priority code against synthetic firmware.
...
def main():
    ...
    output = Path(tempfile.mkdtemp(prefix='boot-target-', dir=parent))
    print(f'Evidence: {output}', flush=True)
    efi = build_harness(output, boot_target=True)
    ...
    for name in ('BootTargetTest.log', 'BootTargetTest.result'):
        content = subprocess.check_output(['mtype', '-i', fat, '::/EFI/BOOT/' + name])
        (output / name).write_bytes(content)
    result = (output / 'BootTargetTest.result').read_text()
